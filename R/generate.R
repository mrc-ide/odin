...
odin_generate <- function(dat, dest=tempdir(), package=FALSE) {
  ...
  struct <- c(odin_generate_struct(obj),
              odin_generate_support_decls(obj))
  ...
  ret <- list(if (!package) odin_header(),
              ...
              odin_generate_support_defns(obj),
...
odin_generate_support_decls <- function(obj) {
  ret <- collector()
...
odin_generate_support_defns <- function(obj) {
  ret <- collector()
  ret$add("%s* %s_get_pointer(SEXP %s_ptr, int closed_error) {",
          obj$type_pars, obj$base, obj$base)
