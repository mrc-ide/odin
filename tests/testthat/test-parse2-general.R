...
test_that("every line contains an assignment", {
  expect_error(odin_parse2_("hello\nfoo"),
               "Every line must contain an assignment")
  expect_error(odin_parse2_(quote(foo ~ bar)),
               "Every line must contain an assignment")
})
...
test_that("expression parsing", {
  expect_error(odin_parse2_(quote(1[1:4] <- 1)),
               "array lhs must be a name")
  expect_error(odin_parse2_(quote(x[f(1)] <- 1)),
               "Invalid function in array calculation")
  expect_error(odin_parse2_(quote(x[c(1, 2)] <- 1)),
               "Invalid function in array calculation")

  expect_error(odin_parse2_(quote(x <- 1 + user(2))),
               "user() must be the only call on the rhs", fixed = TRUE)

  expect_error(odin_parse2_(quote(y <- deriv(x))),
               "Function deriv is disallowed on rhs")

  expect_error(odin_parse2_(quote(initial(x) <- user())),
               "user() only valid for non-special variables", fixed = TRUE)
  expect_error(odin_parse2_(quote(deriv(x) <- user())),
               "user() only valid for non-special variables", fixed = TRUE)
  expect_error(odin_parse2_(quote(x[i] <- y[i])),
...
test_that("parse array indices", {
  ...
  expect_error(
    odin_parse2_(ex("x[1] <- 1\ny[x] <- 1\ndim(x) <- 1\ndim(y) <- 1")),
    "Array indices may not be arrays")
})


test_that("reserved names", {
  expect_error(odin_parse2_(quote(i <- 1)),
               "Reserved name")
  expect_error(odin_parse2_(quote(deriv <- 1)),
               "Reserved name")
  expect_error(odin_parse2_(quote(t <- 1)),
               "Reserved name")
  expect_error(odin_parse2_(quote(dim <- 1)),
               "Reserved name")
  expect_error(odin_parse2_(quote(user <- 1)),
               "Reserved name")

  expect_error(odin_parse2_(quote(deriv_x <- 1)),
               "Variable name cannot start with 'deriv_'")
  expect_error(odin_parse2_(quote(dim_x <- 1)),
               "Variable name cannot start with 'dim_'")
  expect_error(odin_parse2_(quote(initial_x <- 1)),
...
test_that("dim lhs must be simple", {
  expect_error(odin_parse2_(quote(dim(x[1]) <- 1)),
               "must be applied to a name only")
  expect_error(odin_parse2_(quote(dim(x[1,2]) <- c(1, 2))),
               "must be applied to a name only")
})
...
test_that("recursive variables", {
  expect_error(odin_parse2_(quote(foo <- foo + 1)),
               "Self referencing expressions not allowed")
})

test_that("array extent and time", {
  expect_error(odin_parse2_(quote({
    deriv(y[]) <- 1
    initial(y[]) <- 0
    dim(y) <- t
...
test_that("lhs checking", {
  expect_error(odin_parse2_(quote(1 <- 1)),
               "Invalid left hand side")

  expect_error(odin_parse2_(quote(1 + 1 <- 1)),
               "Unhandled expression + on lhs", fixed = TRUE)
  expect_error(odin_parse2_(quote(devs(a) <- 1)),
               "Unhandled expression devs on lhs", fixed = TRUE)

  expect_error(odin_parse2_(quote(deriv(a, b) <- 1)),
               "Invalid length special function on lhs")
  expect_error(odin_parse2_(quote(initial(a, b) <- 1)),
               "Invalid length special function on lhs")
  expect_error(odin_parse2_(quote(dim(a, b) <- 1)),
               "Invalid length special function on lhs")

  expect_error(odin_parse2_(quote(deriv() <- 1)),
               "Invalid length special function on lhs")
  expect_error(odin_parse2_(quote(initial() <- 1)),
               "Invalid length special function on lhs")
  expect_error(odin_parse2_(quote(dim() <- 1)),
               "Invalid length special function on lhs")

  expect_error(odin_parse2_(quote(deriv(deriv(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(deriv(initial(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(deriv(dim(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(initial(deriv(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(initial(initial(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(initial(dim(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(dim(deriv(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(dim(initial(a)) <- 1)),
               "Invalid nested lhs function usage")
  expect_error(odin_parse2_(quote(dim(dim(a)) <- 1)),
               "Invalid nested lhs function usage")
})
...
test_that("interpolation", {
  ...
  expect_error(odin_parse2_(quote(x <- interpolate(a, b, 1L))),
               "Expected a string constant for interpolation type")
  ...
  expect_error(odin_parse2_(quote(x <- interpolate(2, x))),
               "interpolation time argument must be a symbol")
  expect_error(odin_parse2_(quote(x <- interpolate(x, 2))),
               "interpolation target argument must be a symbol")
})

test_that("sums", {
  expect_error(odin_parse2_(quote(x <- sum(1 + 2))),
               "Argument to sum must be a symbol or indexed array")
  expect_error(odin_parse2_(quote(x <- sum(1))),
               "Argument to sum must be a symbol or indexed array")
  expect_error(odin_parse2_(quote(x <- sum(a, b))),
               "Expected 1 argument in sum call, but recieved 2")
  expect_error(odin_parse2_(quote(x <- sum())),
               "Expected 1 argument in sum call, but recieved 0")

  expect_error(odin_parse2_(quote(x <- sum(a[f(b)]))),
               "Invalid array use in sum")
  expect_error(odin_parse2_(quote(x <- sum(a[f(b), c]))),
               "Invalid array use in sum")
  expect_error(odin_parse2_(quote(x <- sum(a[f(b), f(c)]))),
               "Invalid array use in sum")
})


test_that("some dim() pathologies", {
  expect_error(odin_parse2_(ex("a[] <- user(); dim(a) <- user(1)")),
               "Default in user dimension size not handled")
  expect_error(odin_parse2_(ex("dim(a) <- 'foo'")),
               "expected numeric, symbol, user or c")
  expect_error(odin_parse2_(ex("dim(a) <- NULL")),
               "expected numeric, symbol, user or c")
  expect_error(odin_parse2_(ex("dim(a) <- c(1, 'foo')")),
               "must contain symbols, numbers or lengths")
  expect_error(odin_parse2_(ex("dim(a) <- c(1, c(2, 3))")),
               "must contain symbols, numbers or lengths")
  expect_error(odin_parse2_(ex("dim(a) <- c(1, NULL)")),
               "must contain symbols, numbers or lengths")
  expect_error(odin_parse2_(ex("dim(a) <- c(1 + 1, 1)")),
               "must contain symbols, numbers or lengths")
})
...
test_that("array pathologies", {
  expect_error(
    odin_parse2_(ex("dim(y) <- 2; y <- 1")),
    "Array variables must always assign as arrays")
  expect_error(
    odin_parse2_(ex("dim(y) <- user(); y <- 1")),
    "Array variables must always assign as arrays")
  expect_error(
    odin_parse2_(ex("dim(y) <- 2; y[1] <- user(); y[2] <- 1")),
...
test_that("correct dim() use", {
  ...
  expect_error(odin_parse2_(ex("a <- length(1)")),
               "argument to length must be a symbol", fixed = TRUE)
  expect_error(odin_parse2_(ex("x <- 2; a <- length(x)")),
               "argument to length must be an array", fixed = TRUE)
  expect_error(odin_parse2_(ex(
  ...
  expect_error(odin_parse2_(ex("a <- dim(1, x)")),
               "first argument to dim must be a symbol", fixed = TRUE)

  expect_error(odin_parse2_(ex("x <- 2; a <- dim(x, 1)")),
  ...
  expect_error(odin_parse2_(ex("dim(x) <- 10; b <- length(x)")),
               "Array variable x is never assigned")
})

test_that("check array rhs", {
  expect_error(
    odin_parse2_(ex("dim(x) <- 10; x[] <- 1; b <- x[]")),
    "Empty array index not allowed on rhs")

  expect_error(
    odin_parse2_(ex("dim(x) <- 10; x[] <- 1; a <- x[x]")),
    "Disallowed variables used for x")

  expect_error(
    odin_parse2_(ex("dim(x) <- 10; x[] <- 1; y <- 1; a <- x[1] + y[1];")),
...
test_that("rewrite errors", {
  expect_error(odin_parse2_(ex("a <- 1; x <- foo(a)")),
               "Unsupported function: foo")
  expect_error(odin_parse2_(ex("a <- 1; x <- foo(a); y <- bar(a)")),
               "Unsupported functions: foo, bar")
  expect_error(odin_parse2_(ex("a <- 1; x <- abs(a, 1)")),
               "Expected 1 argument in abs call")
  expect_error(odin_parse2_(ex("a <- 1; x <- min(a)")),
