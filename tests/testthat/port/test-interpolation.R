...
test_that("constant", {
  ...
  tp <- c(0, 1, 2)
  zp <- c(0, 1, 0)
  ## TODO: There are lots of bits to get on with here for error checking.
  ## expect_error(gen(tp = tp, zp = zp[1:2]), "Expected 'y' to have length 3")
  ## expect_error(gen(tp = tp, zp = rep(zp, 2)), "Expected 'y' to have length 3")
